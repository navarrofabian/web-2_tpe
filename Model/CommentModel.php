...
class CommentModel
{
    ...
    function getCommentsByRating($id_product, $rating, $orderBy, $asc = false)
    {
        $order_querys = [
            "id_comment" => "ORDER BY id_comment",
            "rating" => "ORDER BY rating",
        ...
        $sql = "SELECT comment.*, user.userName FROM comment LEFT JOIN user ON comment.id_user = user.id_user WHERE id_product = ? AND rating = ? $order_query";
        $query = $this->db->prepare($sql);
