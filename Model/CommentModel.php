...
class CommentModel{
    ...
    function getCommentsByRating($id_product, $rating, $sort, $order)
    {
        $sql = "SELECT comment.*, user.userName FROM comment LEFT JOIN user ON comment.id_user = user.id_user WHERE id_product = ? AND rating = ? ORDER BY $sort $order";
        $query = $this->db->prepare($sql);
    ...
    function insertComment($comment_content, $rating, $id_product, $id_user, $date){
        $sentence = $this->db->prepare("INSERT INTO `comment`(`comment_content`, `rating`, `id_product`, `id_user`, `date`) VALUES(?, ?, ?, ?, ?)");
        $sentence->execute(array($comment_content, $rating, $id_product, $id_user, $date));
